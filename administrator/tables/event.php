...
class EventsTableevent extends JTable
{
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'EventsTableevent', array('typeAlias' => 'com_events.event'));
	...
	public function bind($array, $ignore = '')
	{
		...
		// Support for empty date field: occurrence_date
		if($array['occurrence_date'] == '0000-00-00' )
		{
			$array['occurrence_date'] = '';
		}

		// Support for empty date field: recurrence_until
		if($array['recurrence_until'] == '0000-00-00' )
		{
			$array['recurrence_until'] = '';
		}
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_events.event.' . $array['id']))
		{
			...
			$default_actions = JAccess::getAssetRules('com_events.event.' . $array['id'])->getData();
			$array_jaccess   = array();
