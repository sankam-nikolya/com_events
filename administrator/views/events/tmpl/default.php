		...
		<table class="table table-striped" id="eventList">
			<thead>
				<tr>
					...
					<th class='left'>
						<?php echo JHtml::_('grid.sort',  'COM_EVENTS_EVENTS_CATEGORY', 'a.`category`', $listDirn, $listOrder); ?>
					</th>
					<th class='left'>
						<?php echo JHtml::_('grid.sort',  'COM_EVENTS_EVENTS_START_DATE', 'a.`start_date`', $listDirn, $listOrder); ?>
					</th>
					<th class='left'>
						<?php echo JHtml::_('grid.sort',  'COM_EVENTS_EVENTS_END_DATE', 'a.`end_date`', $listDirn, $listOrder); ?>
					</th>
					<th class='left'>
						<?php echo JHtml::_('grid.sort',  'COM_EVENTS_EVENTS_REPEATABLE', 'a.`repeatable`', $listDirn, $listOrder); ?>
					</th>
					<th class='left'>
						<?php echo JHtml::_('grid.sort',  'COM_EVENTS_EVENTS_OVERRIDE_EVENT', 'a.`override_event`', $listDirn, $listOrder); ?>
					</th>
					<th class='left'>
						<?php echo JHtml::_('grid.sort',  'COM_EVENTS_EVENTS_ID', 'a.`id`', $listDirn, $listOrder); ?>
					</th>
				</tr>
			</thead>
			<tfoot>
			...
			<?php foreach ($this->items as $i => $item) :?>
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td>
					<?php if ($canEdit) : ?>
						<a href="<?php echo JRoute::_('index.php?option=com_events&task=category.edit&id='.(int) $item->category); ?>">
					...
					<td>
						<?php echo date('d.m.Y g:i', strtotime($item->start_date)); ?>
					</td>
					<td>
						<?php echo date('d.m.Y g:i', strtotime($item->end_date)); ?>
					</td>
					<td>
						<?php 
							if($item->repeatable) {
								echo JText::_('JYES') . ' (' . $item->recurrence_type . ')';
							} else {
								echo JText::_('JNO');
							}
						?>
					...
					<td>
						<?php echo ($item->override_event) ? JText::_('JYES') : JText::_('JNO'); ?>
					</td>
					<td>
						<?php echo $item->id; ?>
					</td>
